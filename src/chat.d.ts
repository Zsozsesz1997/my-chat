...
class Proxy extends EventProducer<ProxyEventMap>{
    ...
    constructor() {
        ...
        this.ws.addEventListener("open", () => {
            this.sendPacket({ type: "register", email: "ezegyremektantargy@lba.com", password: "redifgji01201" , displayName: "qwetqq", staySignedIn: true })
        });
        */
        this.ws.addEventListener("message", e => {
            ...
            switch (p.type) {
                case "error":
                    alert(p.message);
                    console.log("EZ EGY HIBA!");
                    break;
                case "login":
                    this.inbox = p.inbox;
                    this.dispatch("login");
                    console.log("HÁT LOGINOLJÁ BEFELE.");
                    break;
